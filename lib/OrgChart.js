...
/***/ function(module, exports) {
	...
	var OrgChart = function () {
	  ...
	  _createClass(OrgChart, [{
	    ...
	    value: function _buildSiblingNode(chart, nodeData, callback) {
	      var _this9 = this;
	
	      var newSiblingCount = nodeData.siblings ? nodeData.siblings.length : nodeData.children.length,
	          existingSibligCount = chart.parentNode.nodeName === 'TD' ? this._closest(chart, function (el) {
	        return el.nodeName === 'TR';
	      }).children.length : 1,
	      ...
	      if (chart.parentNode.nodeName === 'TD') {
	        (function () {
	          var temp = _this9._prevAll(chart.parentNode.parentNode);
	
	          temp[0].remove();
	          temp[1].remove();
	          var childCount = 0;
	
	          _this9._buildChildNode(_this9._closest(chart, function (el) {
	            return el.nodeName === 'TABLE';
	          }), nodeData, function () {
	            var _this10 = this;
	            ...
	            if (++childCount === newSiblingCount) {
	              (function () {
	                var siblingTds = _this10._closest(chart, function (el) {
	                  return el.nodeName === 'TABLE';
	                }).children[3].children;
	
	                if (existingSibligCount > 1) {
	                  Array.from(chart.parentNode.children).forEach(function (el) {
	                    siblingTds[0].parentNode.insertBefore(el, siblingTds[0]);
	                  });
	                  chart.parentNode.parentNode.remove();
	                  _this10._complementLine(siblingTds[0], siblingCount, existingSibligCount);
	                  _this10._addClass(siblingTds, 'hidden');
	                  siblingTds.forEach(function (el) {
	                    _this10._addClass(el.querySelectorAll('.node'), 'slide-left');
	                  });
	                } else {
	                  siblingTds[insertPostion].parentNode.insertBefore(chart.parentNode, siblingTds[insertPostion]);
	                  chart.parentNode.parentNode.remove();
	                  _this10._complementLine(siblingTds[insertPostion], siblingCount, 1);
	                  var _temp = siblingTds.slice(0, insertPostion);
	
	                  _this10._addClass(_temp, 'hidden');
	                  _this10._addClass(_temp.querySelectorAll('.node'), 'slide-right');
	
	                  var temp2 = siblingTds.slice(insertPostion);
	
	                  _this10._addClass(temp2, 'hidden');
	                  _this10._addClass(temp2.querySelectorAll('.node'), 'slide-left');
	                }
	                callback();
	              })();
	            }
	        ...
	        (function () {
	          ...
	          _this9.buildHierarchy(nodeData, 0, function () {
	            if (++nodeCount === siblingCount) {
	              var _temp2 = chart.nextElementSibling.children[3].children[insertPostion],
	                  td = document.createElement('td');
	
	              td.setAttribute('colspan', 2);
	              td.appendChild(chart);
	              _temp2.parentNode.insertBefore(td, _temp2.nextElementSibling);
	              _this9._complementLine(_temp2, siblingCount, 1);
	
	              var temp2 = _this9._closest(chart, function (el) {
	                return el.nodeName === 'TABLE';
	              }).children[0];
	
	              temp2.classList.add('hidden');
	              _this9._addClass(temp2.querySelectorAll('.node'), 'slide-down');
	
	              var temp3 = _this9._siblings(chart.parentNode);
	
	              _this9._addClass(temp3, 'hidden');
	              temp3.slice(0, insertPostion).forEach(function (el) {
	                _this9._addClass(el.querySelectorAll('.node'), 'slide-right');
	              });
	              temp3.slice(insertPostion).forEach(function (el) {
	                _this9._addClass(el.querySelectorAll('.node'), 'slide-left');
	              });
	              callback();
	            }
	          });
	    ...
	    value: function addSiblings(node, data) {
	      ...
	      this._buildSiblingNode(this._closest(node, function (el) {
	        return el.nodeName === 'TABLE';
	      }), data, function () {
	        _this11._closest(node, function (el) {
	          return el.classList.contains('.nodes');

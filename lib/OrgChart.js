	...
	var OrgChart = function () {
	  function OrgChart(options) {
	    ...
	    if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') {
	      ...
	      this._getJSON(data).then(function (resp) {
	        that.buildHierarchy(chart, opts.ajaxURL ? JSON.parse(resp) : that._attachRel(JSON.parse(resp), '00'), 0);
	      }).catch(function (err) {
	  ...
	  _createClass(OrgChart, [{
	    ...
	    value: function _endLoading(arrow, node) {
	      ...
	      this.chart.dataset.inAjax = false;
	      this.chartContainer.querySelector('.oc-export-btn' + (opts.chartClass !== '' ? '.' + opts.chartClass : '')).disabled = false;
	    }
